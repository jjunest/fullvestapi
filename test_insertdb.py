        ...
        for i in range(len(stock_list_kor)):
            ...
            if stock_detail_soup.find("span", id="time") is not None:
                ...
                if len(stock_short_info_table1_em) > 2:
                    stock_foreign_share_percent = remove_comma_string(
                        stock_short_info_table1_em[2].get_text().strip().replace("%", ""))
                ...
                if len(stock_trend_table) > 0:
                    ...
                    if len(stock_trend_table[0].select("tfoot td em")) != 0:
                        stock_foreign_buy_today = remove_comma_string(
                            stock_trend_table[0].select("tfoot td em")[0].get_text())
                        stock_foreign_sell_today = remove_comma_string(
                            stock_trend_table[0].select("tfoot td em")[1].get_text())
                        stock_foreign_total_today = remove_comma_string(
                            stock_trend_table[0].select("tfoot td em")[2].get_text())
                    ...
                    if len(stock_trend_6days_em) != 0:
                        for i in range(0, 6, 1):
                            ...
                            if len(stock_trend_table[1].find_all(attrs={'scope': 'row'})) > i:
                                stock_trend_6days_datetime = stock_trend_table[1].find_all(attrs={'scope': 'row'})[
                                ...
                                if info_date.strftime("%m-%d") == stock_trend_6days_datetime:
                                    ...
                                    stock_trading_sum_foreign = remove_comma_string(
                                        stock_trend_6days_em[i * 4 + 2].get_text().strip())
                                    stock_trading_sum_agency = remove_comma_string(
                                        stock_trend_6days_em[i * 4 + 3].get_text().strip())
                                    ...
                                    if stock_trading_sum_foreign != '' and stock_trading_sum_agency != '':
                                        stock_trading_sum_ant = - int(stock_trading_sum_foreign) - int(
                ...
                stock_summary_info_dataframe = stock_summary_info_dataframe.append(stock_summary_info,
                                                                                   ignore_index=True)
                print("this is summary_info_list len:", len(stock_summary_info_dataframe))
                # 100 개씩 잘라서 넣어주고, dataframe을 초기화 시켜줘야됨
                stock_summary_info_dataframe_csv = stock_summary_info_dataframe_csv.append(stock_summary_info,
                                                                                           ignore_index=True)
